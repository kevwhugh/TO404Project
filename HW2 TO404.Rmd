...
morningSum %>% arrange(desc(overflow)) %>% head()



#Stations with Most Overflow in Morning 
ggplot(head(morningSum[order(morningSum$overflow, decreasing = TRUE),], 5), aes(x = start.station.name, y = overflow)) + geom_col()

# Stations with Most Underflow in Morning 
# ?????????????????????????? what is going on with the # of bars being shown 
ggplot(tail(morningSum[order(morningSum$overflow, decreasing = TRUE),], 5), aes(x = start.station.name, y = overflow)) + geom_col()
```
...
ggplot(head(afternoonSum[order(afternoonSum$overflow, decreasing = TRUE),], 5), aes(x = start.station.name, y = overflow)) + geom_col()

# Stations with Most Underflow in Early Afternoon 
ggplot(tail(afternoonSum[order(afternoonSum$overflow, decreasing = TRUE),], 5), aes(x = start.station.name, y = overflow)) + geom_col()
```
...
ggplot(head(rush_hourSum[order(rush_hourSum$overflow, decreasing = TRUE),], 5), aes(x = start.station.name, y = overflow)) + geom_col()

# Stations with Most Underflow During Afternoon Rush Hour 
ggplot(tail(rush_hourSum[order(rush_hourSum$overflow, decreasing = TRUE),], 5), aes(x = start.station.name, y = overflow)) + geom_col()
```
...
ggplot(head(nightSum[order(nightSum$overflow, decreasing = TRUE),], 5), aes(x = start.station.name, y = overflow)) + geom_col()

# Stations with Most Underflow at Night 
ggplot(tail(nightSum[order(nightSum$overflow, decreasing = TRUE),], 5), aes(x = start.station.name, y = overflow)) + geom_col()
```
...
ggplot(head(days[order(days$overflow, decreasing = TRUE),], 5), aes(x = id, y = overflow)) + geom_col()

# Stations with Most Underflow per day
ggplot(tail(days[order(days$overflow, decreasing = TRUE),], 5), aes(x = id, y = overflow)) + geom_col()
```
...
#head(allHour)
m1<-lm(allHour$overflow ~ allHour$hourStop) 
m2<-lm(allHour$overflow ~ allHour$hourStart) #times of start and stop affect overflow 
m3<-lm(days$overflow ~ days$day.x) # day does not affect overflow much
summary(m1); summary(m2); summary(m3)

ggplot(allHour, aes(hourStart, overflow, group = hourStart)) + geom_boxplot() 
ggplot(allHour, aes(hourStop, overflow, group = hourStop)) + geom_boxplot() 
ggplot(days, aes(day.x, overflow, group = day.x)) + geom_boxplot() 
```
