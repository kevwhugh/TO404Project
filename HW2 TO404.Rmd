...
morning <- merge(a1, b1, by.x = 'id', by.y = 'id', all.x = TRUE) %>% group_by(id, start.station.latitude, start.station.longitude, end.station.latitude, end.station.longitude) %>% mutate(nnn = ifelse(is.na(nnn), 0, nnn)) %>% summarize(overflow = nnn-nn)
#select(id, nn, nnn) %>%
print(morning)
#morning  %>% arrange(desc(overflow))
