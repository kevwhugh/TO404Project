...
morning <- morning %>% group_by(start.station.name) %>% summarize(overflow = sum(overflow))
morning %>% arrange(desc(overflow)) %>% head()
...
ggplot(tail(morning[order(morning$overflow, decreasing = TRUE),], 5), aes(x = start.station.name, y = overflow)) + geom_col()
```
...
afternoon <- afternoon %>% group_by(start.station.name) %>% summarize(overflow = sum(overflow))
afternoon %>% arrange(desc(overflow)) %>% head()
...
rush_hour <- rush_hour %>% group_by(start.station.name) %>% summarize(overflow = sum(overflow))
rush_hour %>% arrange(desc(overflow)) %>% head()
...
night <- night %>% group_by(start.station.name) %>% summarize(overflow = sum(overflow))
night %>% arrange(desc(overflow)) %>% head()
